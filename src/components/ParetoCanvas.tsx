...
export default function ParetoCanvas() {
  ...
  const {  currentProjectId, pareto, addParetoProject, addParetoItem, updateParetoItem, deleteParetoItem, updateParetoTitle  } = useRoadmapStore(useShallow((state) => ({
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
          {isEditingTitle && activePareto && currentProjectId ? (
             <input 
               autoFocus
               type="text"
               value={activePareto.title}
               onChange={(e) => updateParetoTitle(currentProjectId, activePareto.id, e.target.value)}
