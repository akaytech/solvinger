    ...
    >
      {isEditing ? (
        ...
        <div className="py-1 max-h-80 overflow-y-auto custom-scrollbar">
          ...
          <button onClick={() => onAddNode('event', t('fta_new_event'))} className="w-full px-4 py-1.5 text-left text-sm text-amber-600 dark:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/20 flex items-center gap-2 transition-colors">
            <Box size={16} /> {t('fta_add_event')}
          </button>
          <button onClick={() => onAddNode('basicEvent', t('fta_new_basic'))} className="w-full px-4 py-1.5 text-left text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 flex items-center gap-2 transition-colors">
            <CircleDot size={16} /> {t('fta_add_basic')}
          </button>
          <button onClick={() => onAddNode('undevelopedEvent', t('fta_new_undeveloped') || 'Yeni Geliştirilmemiş Olay')} className="w-full px-4 py-1.5 text-left text-sm text-orange-600 dark:text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 flex items-center gap-2 transition-colors">
            <Diamond size={16} /> {t('fta_add_undeveloped') || 'Geliştirilmemiş Olay'}
          </button>
          <button onClick={() => onAddNode('conditioningEvent', t('fta_new_conditioning') || 'Yeni Koşul')} className="w-full px-4 py-1.5 text-left text-sm text-lime-600 dark:text-lime-500 hover:bg-lime-50 dark:hover:bg-lime-900/20 flex items-center gap-2 transition-colors">
          ...
          <button onClick={() => onAddNode('andGate', t('fta_add_and'))} className="w-full px-4 py-1.5 text-left text-sm text-indigo-600 dark:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 flex items-center gap-2 transition-colors">
            <span className="font-black text-lg w-4 text-center">&amp;</span> {t('fta_add_and')}
          </button>
          <button onClick={() => onAddNode('priorityAndGate', t('fta_add_priority_and') || 'Priority AND Ekle')} className="w-full px-4 py-1.5 text-left text-sm text-violet-600 dark:text-violet-500 hover:bg-violet-50 dark:hover:bg-violet-900/20 flex items-center gap-2 transition-colors">
            <ListOrdered size={16} /> {t('fta_add_priority_and') || 'Priority AND Kapısı'}
          </button>
          <button onClick={() => onAddNode('orGate', t('fta_add_or'))} className="w-full px-4 py-1.5 text-left text-sm text-teal-600 dark:text-teal-500 hover:bg-teal-50 dark:hover:bg-teal-900/20 flex items-center gap-2 transition-colors">
            <span className="font-black text-sm w-4 text-center">≥1</span> {t('fta_add_or')}
          </button>
          <button onClick={() => onAddNode('exclusiveOrGate', t('fta_add_xor') || 'XOR Ekle')} className="w-full px-4 py-1.5 text-left text-sm text-cyan-600 dark:text-cyan-500 hover:bg-cyan-50 dark:hover:bg-cyan-900/20 flex items-center gap-2 transition-colors">
            <GitBranch size={16} /> {t('fta_add_xor') || 'Exclusive OR (XOR)'}
          </button>
          <button onClick={() => onAddNode('inhibitGate', t('fta_add_inhibit') || 'Inhibit Ekle')} className="w-full px-4 py-1.5 text-left text-sm text-fuchsia-600 dark:text-fuchsia-500 hover:bg-fuchsia-50 dark:hover:bg-fuchsia-900/20 flex items-center gap-2 transition-colors">
