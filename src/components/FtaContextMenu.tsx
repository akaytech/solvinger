    ...
    >
      {isEditing ? (
        ...
        <div className="py-1">
          ...
          <button onClick={() => onAddNode('event', t('fta_new_event'))} className="w-full px-4 py-2 text-left text-sm text-amber-600 dark:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/20 flex items-center gap-2 transition-colors">
            <Box size={16} /> {t('fta_add_event')}
          </button>
          <button onClick={() => onAddNode('andGate', t('fta_add_and'))} className="w-full px-4 py-2 text-left text-sm text-indigo-600 dark:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 flex items-center gap-2 transition-colors">
            <span className="font-black text-lg w-4 text-center">&</span> {t('fta_add_and')}
          </button>
          <button onClick={() => onAddNode('orGate', t('fta_add_or'))} className="w-full px-4 py-2 text-left text-sm text-teal-600 dark:text-teal-500 hover:bg-teal-50 dark:hover:bg-teal-900/20 flex items-center gap-2 transition-colors">
            <span className="font-black text-sm w-4 text-center">≥1</span> {t('fta_add_or')}
          </button>
          <button onClick={() => onAddNode('basicEvent', t('fta_new_basic'))} className="w-full px-4 py-2 text-left text-sm text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 flex items-center gap-2 transition-colors">
